      ...
      <el-row v-for="(item, index) in list" :key="index">
        <el-card class="box-card">
          <div slot="header" class="clearfix">
              <span>规则名</span>
              <el-input style="width: 200px" class="filter-item" placeholder="必选" v-model="list[index].name"></el-input>
          </div>
          <el-button size="mini" type="primary" @click="handleAddRelation(index)" >增加表达式</el-button>
          <el-button type="danger" size="mini" @click="handleDeleteRelation(index)" >删除表达式</el-button>
        ...
        <span class="custom-tree-node">
          ...
          <span>
            <el-button
              size="mini"
              type="text"
              on-click={() => this.appendExpression(data)}
            >
              增加子表达式
            </el-button>
            <el-button
              size="mini"
              type="text"
              on-click={() => this.removeExpression(node, data)}
            >
              删除该表达式
            </el-button>
          </span>
